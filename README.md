...
Drizzle schema files are the source of truth, and ordered SQL migrations are committed under `src/db/migrations`. Production schema push is unsupported. The Worker never runs migrations during module initialization, requests, or health checks. Until the automated migration-first release workflow is added, operators must preserve the order above so a migration failure prevents deployment.
